"""Strict evidence contracts for the one-time detector-v4 blind evaluation."""
...
from retryrail.detection.v4_models import V4ScopeArbitration
...
class V4BlindReleaseStatus(StrEnum):
...
class V4BlindReleaseTarget(StrEnum):
    """Stable identifiers for every detector-v4 release target."""
...
class V4BlindFailureStage(StrEnum):
...
class V4BlindProcedureFreeze(StrictContract):
    """Pre-nonce identity for the already-frozen candidate and blind runner."""

    schema_version: Literal["1.0.0"] = "1.0.0"
    freeze_id: Literal["detector_v4_blind_procedure_freeze_v1"] = (
        "detector_v4_blind_procedure_freeze_v1"
    )
    ...
    candidate_freeze_id: Literal["detector_v4_candidate_freeze_v1"] = (
    ...
    runner_source_paths: tuple[ArtifactPath, ...] = Field(min_length=3)
...
class V4BlindPredictionArtifact(V2PredictionArtifact):
    """Label-free detector-v4 output persisted before truth authorization."""
    ...
    def validate_arbitration_references(self) -> Self:
        """Require unique losers that reference exactly one emitted incident."""
...
class V4BlindPredictionReceipt(StrictContract):
...
class V4BlindTruthAccessReceipt(StrictContract):
    """Marker written only after prediction bytes and hashes are re-verified."""

    schema_version: Literal["1.0.0"] = "1.0.0"
    receipt_id: Literal["detector_v4_official_blind_truth_access_receipt_v1"] = (
        "detector_v4_official_blind_truth_access_receipt_v1"
    )
    run_id: str = Field(pattern=r"^detector_v4_official_blind_[a-f0-9]{20}$")
...
class V4BlindReportContractProof(StrictContract):
    """Evidence that required-nullable blind report bytes are canonical."""
...
class V4BlindReport(StrictContract):
    """Official blind scorecard derived by the frozen matcher after prediction."""

    schema_version: Literal["4.0.0"] = "4.0.0"
    report_id: Literal["detector_v4_official_blind_report_v1"] = (
        "detector_v4_official_blind_report_v1"
    )
    run_id: str = Field(pattern=r"^detector_v4_official_blind_[a-f0-9]{20}$")
    protocol_id: Literal["detector_v4_protocol_v1"] = "detector_v4_protocol_v1"
    detector_version: Literal["detector_v4_0_0"] = "detector_v4_0_0"
    ...
    def validate_release_summary(self) -> Self:
        ...
        if self.report_contract.open_incident_ids != open_incident_ids:
            msg = "blind report open-incident proof must match incident summaries"
            raise ValueError(msg)
        return self


class V4BlindReleaseDecision(StrictContract):
    """Fail-closed decision that still requires M4 before runtime activation."""

    schema_version: Literal["1.0.0"] = "1.0.0"
    decision_id: Literal["detector_v4_official_blind_release_v1"] = (
        "detector_v4_official_blind_release_v1"
    )
    run_id: str = Field(pattern=r"^detector_v4_official_blind_[a-f0-9]{20}$")
    source_report_id: Literal["detector_v4_official_blind_report_v1"] = (
        "detector_v4_official_blind_report_v1"
    )
    ...
    failed_targets: tuple[V4BlindReleaseTarget, ...]
    release_qualified: bool
    approved_for_m4_integration: bool
    runtime_action_eligible: Literal[False] = False
    ...
    def validate_decision(self) -> Self:
        ...
        if len(set(self.failed_targets)) != len(self.failed_targets):
            ...
            raise ValueError(msg)
        qualified = self.status is V4BlindReleaseStatus.QUALIFIED
...
class V4BlindNonceReveal(StrictContract):
...
class V4BlindCompletionReceipt(StrictContract):
    ...
    artifacts: tuple[ArtifactDigest, ...] = Field(min_length=10, max_length=10)
    completed_at: AwareDatetime
    predictions_persisted_before_truth: Literal[True] = True
    report_strict_reload_verified: Literal[True] = True
    report_canonical_byte_round_trip_verified: Literal[True] = True
    official_blind_evaluated: Literal[True] = True
    release_qualified: bool
    approved_for_m4_integration: bool
    runtime_action_eligible: Literal[False] = False
...
class V4BlindFailureReceipt(StrictContract):
    ...
    nonce_sha256: Sha256Digest
    failed_stage: V4BlindFailureStage
