"""Reproduce revealed detector-v4 blind inputs from append-only receipts."""
...
from retryrail.detection.v4_blind import check_blind_procedure
...
class V4BlindReproductionError(RuntimeError):
    """A revealed run cannot be reproduced without violating its receipts."""


@dataclass(frozen=True, slots=True)
class V4BlindReproductionSummary:
...
def _artifact_by_path(
    receipt: V4BlindCompletionReceipt,
    expected_path: str,
) -> ArtifactDigest:
...
def _evidence_matches(
    ...
    return (
        digest.sha256 == _sha256(content)
        and digest.bytes == len(content)
        and digest.records == 1
    )
...
def _reproduce_run(run_directory: Path, *, root: Path) -> tuple[int, int]:
    ...
    identities_match = (
        ...
        and commitment.detector_config_sha256 == prediction.detector_config_sha256
        and commitment.candidate_bundle_sha256 == prediction.candidate_bundle_sha256
        and commitment.runner_bundle_sha256 == prediction.runner_bundle_sha256
    ...
    existing = tuple(
        _verify_existing(target, artifact.content, root=root)
        for target, artifact in artifacts
    )
    created = 0
    for (target, artifact), already_exists in zip(artifacts, existing, strict=True):
        if not already_exists and _create_new(target, artifact.content, root=root):
            created += 1
...
def reproduce_revealed_blind_inputs(
    ...
    if root == _REPOSITORY_ROOT:
        ...
        if findings:
            raise V4BlindReproductionError("; ".join(findings))
    ...
    return V4BlindReproductionSummary(
...
def main() -> None:
    ...
    try:
        summary = reproduce_revealed_blind_inputs(arguments.root)
    except (OSError, V4BlindReproductionError) as error:
        sys.stderr.write(f"detector-v4 blind reproduction failed: {error}\n")
        raise SystemExit(1) from error
    sys.stdout.write(
        "detector-v4 revealed inputs verified: "
