"""Strict evidence contracts for the one-time detector-v2 blind evaluation."""
...
class V2BlindReleaseStatus(StrEnum):
...
class V2BlindReleaseTarget(StrEnum):
    """Stable identifiers for every detector-v2 release target."""
...
class V2BlindFailureStage(StrEnum):
...
class V2BlindProcedureFreeze(StrictContract):
    """Pre-nonce identity for the already-frozen candidate and blind runner."""

    schema_version: Literal["1.0.0"] = "1.0.0"
    freeze_id: Literal["detector_v2_blind_procedure_freeze_v1"] = (
        "detector_v2_blind_procedure_freeze_v1"
    )
    ...
    candidate_freeze_id: Literal["detector_v2_candidate_freeze_v1"] = (
...
class V2BlindNonceCommitment(StrictContract):
    """Durable nonce digest written before any blind event is generated."""

    schema_version: Literal["1.0.0"] = "1.0.0"
    commitment_id: str = Field(pattern=r"^commitment_[a-f0-9]{20}$")
    run_id: str = Field(pattern=r"^detector_v2_official_blind_[a-f0-9]{20}$")
    status: Literal["nonce_committed_before_generation"] = (
        "nonce_committed_before_generation"
    )
    protocol_id: Literal["detector_v2_protocol_v1"] = "detector_v2_protocol_v1"
    protocol_sha256: Sha256Digest
    candidate_freeze_sha256: Sha256Digest
...
class V2BlindPredictionReceipt(StrictContract):
    ...
    run_id: str = Field(pattern=r"^detector_v2_official_blind_[a-f0-9]{20}$")
    status: Literal["predictions_persisted_truth_unopened"] = (
        "predictions_persisted_truth_unopened"
    )
...
class V2BlindTruthAccessReceipt(StrictContract):
...
class V2BlindReport(StrictContract):
    """Official blind scorecard derived by the frozen matcher after prediction."""

    schema_version: Literal["2.0.0"] = "2.0.0"
    report_id: Literal["detector_v2_official_blind_report_v1"] = (
        "detector_v2_official_blind_report_v1"
    )
    run_id: str = Field(pattern=r"^detector_v2_official_blind_[a-f0-9]{20}$")
    protocol_id: Literal["detector_v2_protocol_v1"] = "detector_v2_protocol_v1"
    detector_version: str = Field(pattern=r"^[A-Za-z0-9_-]+$")
...
class V2BlindReleaseDecision(StrictContract):
    """Fail-closed decision that still requires M4 before runtime activation."""

    schema_version: Literal["1.0.0"] = "1.0.0"
    decision_id: Literal["detector_v2_official_blind_release_v1"] = (
        "detector_v2_official_blind_release_v1"
    )
    run_id: str = Field(pattern=r"^detector_v2_official_blind_[a-f0-9]{20}$")
    source_report_id: Literal["detector_v2_official_blind_report_v1"] = (
        "detector_v2_official_blind_report_v1"
    )
    ...
    nonce_sha256: Sha256Digest
    evaluated_at: AwareDatetime
    status: V2BlindReleaseStatus
    failed_targets: tuple[V2BlindReleaseTarget, ...]
    release_qualified: bool
    approved_for_m4_integration: bool
    runtime_action_eligible: Literal[False] = False
    ...
    def validate_decision(self) -> Self:
        ...
        if len(set(self.failed_targets)) != len(self.failed_targets):
            ...
            raise ValueError(msg)
        qualified = self.status is V2BlindReleaseStatus.QUALIFIED
...
class V2BlindNonceReveal(StrictContract):
...
class V2BlindCompletionReceipt(StrictContract):
    ...
    run_id: str = Field(pattern=r"^detector_v2_official_blind_[a-f0-9]{20}$")
    status: Literal["official_blind_evaluation_complete"] = (
        "official_blind_evaluation_complete"
    )
    nonce_sha256: Sha256Digest
    procedure_freeze_sha256: Sha256Digest
    prediction_receipt_sha256: Sha256Digest
...
class V2BlindFailureReceipt(StrictContract):
    ...
    nonce_sha256: Sha256Digest
    failed_stage: V2BlindFailureStage
