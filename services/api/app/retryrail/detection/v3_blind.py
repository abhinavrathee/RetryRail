"""Append-only orchestration for the official detector-v3 blind evaluation."""
...
_CANDIDATE_FREEZE_PATH = _REPOSITORY_ROOT / "evals/golden/detector_v3.freeze.json"
_PROCEDURE_FREEZE_PATH = _REPOSITORY_ROOT / "evals/golden/detector_v3.blind_procedure.freeze.json"
...
_RUNNER_SOURCE_PATHS = (
    "services/api/app/retryrail/detection/v3_blind.py",
    "services/api/app/retryrail/detection/v3_blind_models.py",
)
...
def _load_contract[ContractT: StrictContract](
    ...
    try:
        ...
        raise V3BlindIntegrityError(msg) from error
    if content != _canonical_json(value):
        msg = f"non-canonical {_display_path(path)}"
        raise V3BlindIntegrityError(msg)
    return value
...
def _run_paths(run_id: str, root: Path = _REPOSITORY_ROOT) -> V3BlindRunPaths:
    evidence = root / _RUNS_DIRECTORY / run_id
    generated = root / _GENERATED_DIRECTORY / run_id
    return V3BlindRunPaths(
        run_id=run_id,
...
def _write_new_durable(
    ...
    *,
    ...
    try:
        ...
        raise V3BlindStateError(msg) from error
...
def _procedure_freeze() -> tuple[V3BlindProcedureFreeze, str]:
    content = _PROCEDURE_FREEZE_PATH.read_bytes()
    return (
        V3BlindProcedureFreeze.model_validate_json(content),
        _sha256(content),
    )
...
def _ensure_static_preflight() -> None:
    ...
    if findings:
        raise V3BlindIntegrityError("; ".join(findings))


def _validate_official_nonce(nonce: str) -> str:
    protocol = V3EvaluationProtocol.model_validate_json(_PROTOCOL_PATH.read_bytes())
    ...
    if not blind_procedure.official_nonce_required:
        ...
        raise V3BlindIntegrityError(msg)
    if not blind_procedure.nonce_created_after_candidate_and_runner_freeze:
    ...
    if digest in blind_procedure.prior_and_test_nonce_sha256:
        msg = "prior official and committed test nonces cannot be reused"
        raise ValueError(msg)
    return digest
...
def _assert_prediction_slot(root: Path) -> None:
    ...
    for commitment_path in commitment_paths:
        commitment = _load_contract(commitment_path, V3BlindNonceCommitment)
        ...
        if paths.failure_receipt.is_file():
            ...
            raise V3BlindStateError(msg)
        msg = "an official blind run is already active or complete"
        raise V3BlindStateError(msg)


def _assert_run_paths_available(paths: V3BlindRunPaths) -> None:
...
def _record_failure(
    paths: V3BlindRunPaths,
    *,
    nonce_sha256: str,
    stage: V3BlindFailureStage,
...
def _predict_runtime(
    *,
    ...
    """Run the frozen v3 detector without accepting any truth input."""

    if dataset_role is not V2DatasetRole.BLIND:
        msg = "official v3 prediction requires the blind dataset role"
        raise V3BlindIntegrityError(msg)
    ...
    run = DetectorV3Engine(config).run(
        events,
        partition_started_at=starts_at,
        partition_ended_at=ends_at,
    )
    artifact = V3BlindPredictionArtifact(
...
def _validate_prediction_identity(
    ...
    procedure: V3BlindProcedureFreeze,
) -> None:
    try:
        V3BlindPredictionArtifact.model_validate(prediction.model_dump(mode="python"))
    except ValueError as error:
        msg = "prediction does not satisfy the frozen detector-v3 contract"
        raise V3BlindIntegrityError(msg) from error
    if (
...
def _verify_prediction_readback(path: Path, expected: bytes) -> bytes:
    ...
    if persisted != expected:
        ...
        raise V3BlindIntegrityError(msg)
    V3BlindPredictionArtifact.model_validate_json(persisted)
    return persisted
...
def persist_blind_predictions(
    ...
    *,
    ...
    commitment = V3BlindNonceCommitment(
    ...
    try:
        prediction_lock = output_root / "evals/blind/detector_v3/.prediction.lock"
        ...
        receipt = V3BlindPredictionReceipt(
        ...
        if commitment_written:
            _record_failure(
                paths,
                nonce_sha256=nonce_sha256,
                stage=V3BlindFailureStage.PREDICTION,
...
def _assert_scoring_state(paths: V3BlindRunPaths) -> None:
...
def _require_digest(
    ...
    *,
    ...
    if finding is not None:
        raise V3BlindIntegrityError(finding)
...
def _failed_targets(report: V3BlindReport) -> tuple[V3BlindReleaseTarget, ...]:
    comparisons = (
        ...
        (
            V3BlindReleaseTarget.BASELINE_LEAKAGE,
...
def _build_release_decision(
    ...
    *,
    ...
    return V3BlindReleaseDecision(
        ...
        status=(V3BlindReleaseStatus.QUALIFIED if qualified else V3BlindReleaseStatus.BLOCKED),
...
def _score_blind_locked(
    ...
    *,
    ...
    try:
        procedure, procedure_sha256 = _procedure_freeze()
        commitment = _load_contract(paths.nonce_commitment, V3BlindNonceCommitment)
        ...
        prediction_receipt_sha256 = _sha256(paths.prediction_receipt.read_bytes())
        truth_access = V3BlindTruthAccessReceipt(
        ...
        _record_failure(
            paths,
            nonce_sha256=nonce_sha256,
            stage=V3BlindFailureStage.SCORING,
...
def render_blind_procedure_freeze(
    ...
    """Render the deterministic, nonce-free M3R.4 procedure identity."""
    ...
    candidate = V3CandidateFreeze.model_validate_json(candidate_content)
    return V3BlindProcedureFreeze(
...
def check_blind_procedure() -> list[str]:
    ...
    findings = [*check_v2_artifacts(), *check_v3_protocol(), *check_candidate_freeze()]
...
def _check_prediction_state(
    ...
    *,
    ...
    if not paths.prediction_receipt.is_file():
        ...
        return findings
    try:
        receipt = _load_contract(paths.prediction_receipt, V3BlindPredictionReceipt)
    except V3BlindIntegrityError as error:
...
def _check_completed_links(
    ...
    *,
    ...
    completion: V3BlindCompletionReceipt,
    report: V3BlindReport,
    release: V3BlindReleaseDecision,
    reveal: V3BlindNonceReveal,
    manifest: V2DatasetManifest,
    prediction: V3BlindPredictionArtifact,
    prediction_receipt: V3BlindPredictionReceipt,
    truth_access: V3BlindTruthAccessReceipt,
) -> list[str]:
    findings: list[str] = []
...
def _check_completed_state(
    ...
    *,
    ...
    try:
        completion = _load_contract(paths.completion_receipt, V3BlindCompletionReceipt)
        report = _load_contract(paths.report, V3BlindReport)
        release = _load_contract(paths.release_decision, V3BlindReleaseDecision)
        reveal = _load_contract(paths.nonce_reveal, V3BlindNonceReveal)
        manifest = _load_contract(paths.dataset_manifest, V2DatasetManifest)
        prediction = _load_contract(paths.predictions, V3BlindPredictionArtifact)
        prediction_receipt = _load_contract(
            paths.prediction_receipt,
            V3BlindPredictionReceipt,
        )
...
def _check_failed_state(
    ...
    try:
        failure = _load_contract(paths.failure_receipt, V3BlindFailureReceipt)
    except V3BlindIntegrityError as error:
        findings.append(str(error))
    else:
        ...
        if (
            failure.failed_stage is V3BlindFailureStage.PREDICTION
...
def _check_commitment_procedure_binding(
    ...
    """Verify that historical evidence still names the frozen v3 procedure."""
...
def check_official_blind_artifacts(
    ...
    *,
    ...
    procedure_binding: tuple[V3BlindProcedureFreeze, str] | None = None
    if include_static and not findings:
        procedure_binding = _procedure_freeze()
    prediction_lock = root / "evals/blind/detector_v3/.prediction.lock"
...
def blind_state_summary(root: Path = _REPOSITORY_ROOT) -> str:
    ...
    completed = sum(
        _run_paths(
            V3BlindNonceCommitment.model_validate_json(path.read_bytes()).run_id,
            root,
